...
my $cfg2 = Bio::EnsEMBL::VEP::Config->new({%{$test_cfg->base_testing_cfg}, buffer_size => 100});
$p = Bio::EnsEMBL::VEP::Parser::VCF->new({config => $cfg2, file => $test_cfg->{not_ord_vcf}, valid_chromosomes => [1,21,22]});
$ib = Bio::EnsEMBL::VEP::InputBuffer->new({config => $cfg2, parser => $p, max_not_ordered_variants => $max_non_ordered_variants});
